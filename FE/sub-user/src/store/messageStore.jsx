...
export default function MessageContextProvider({ children }) {
  ...
  async function handleGetAllReceivedMessages(inputStart = null, inputEnd = null, order = "desc") {
    ...
    try {
      const response = await request(`${userProgressStore.DEV_API_URL}/messages/new?start=${inputStart}&end=${inputEnd}&order=${order}`)

      const resData = response.data

      if (response.success) {
        if (resData.message === "New received messages retrieved successfully") {
          return {
            success: true,
            data: resData.result,
          }
  ...
  async function handleGetAllSentMessage(inputStart = null, inputEnd = null, order = "desc") {
    const { start, end } = getOneYearRangeUTC()
    ...
    try {
      const response = await request(`${userProgressStore.DEV_API_URL}/messages/sent?start=${inputStart}&end=${inputEnd}&order=${order}`)
  ...
  async function getNewMessages(inputStart = null, inputEnd = null, order = "desc") {
    const { start, end } = getOneYearRangeUTC()
  ...
  async function insertPhotoFile(formData) {
    try {
      const response = await fetch(`${userProgressStore.IMAGE_API_URL}/upload`, {
        method: "POST",
        body: formData,
  ...
  async function handleSendMessage(content, imageUrl = null) {
    try {
      const response = await request(`${userProgressStore.DEV_API_URL}/messages/send`, "POST", {
