...
export default function UserProgressContextProvider({ children }) {
  ...
  if (IMAGE_API_URL === undefined) IMAGE_API_URL = getEnvironments("IMAGE_API_URL")
  ...
  async function handleCheckEmail(email) {
    try {
      const response = await request(`${DEV_API_URL}/accounts/check-email`, "POST", { email: email })
  ...
  async function handleSignUp(payload) {
    try {
      const response = await request(`${DEV_API_URL}/accounts`, "POST", payload)
  ...
  async function handleUpdateUserInfo(payload) {
    const isConfirmed = window.confirm('정말 회원 정보를 수정하시겠습니까?\n"확인"을 누르면 수정이 완료됩니다.')
    if (!isConfirmed) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/accounts/${loginUserInfo.userInfo.id}`, "PATCH", payload)
  ...
  async function handleSignOut(password) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/accounts/${loginUserInfo.userInfo.id}`, "DELETE", { password: password })
  ...
  async function handleCheckFamilyExist(userId) {
    try {
      const response = await request(`${DEV_API_URL}/families/check-exist`, "POST", { id: userId })
  ...
  async function handleCreateFamily(payload) {
    const isConfirmed = window.confirm(`${payload.family_name}으로 가족 모임 생성을 하시겠습니까?\n확인을 누르면 가족이 생성됩니다.`)
    if (!isConfirmed) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/families`, "POST", payload)
  ...
  async function handleUpdateFamilyInfo(newFamilyName) {
    const isConfirmed = window.confirm("정말 가족 모임 정보를 변경하시겠습니까?\n확인을 누르면 변경됩니다.")
    if (!isConfirmed) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/families/${familyInfo.familyInfo.id}`, "PATCH", { family_name: newFamilyName })
  ...
  async function handleKickMember(memberId, password) {
    try {
      const response = await request(`${DEV_API_URL}/members/kick/${memberId}`, "DELETE", { password: password })
  ...
  async function handleDeleteFamilyInfo(password) {
    const isConfirmed = window.confirm("정말 가족 모임을 삭제하시겠습니까?\n삭제 시 복구가 불가능합니다.")
    if (!isConfirmed) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/families/${familyInfo.familyInfo.id}`, "DELETE", { password: password })
  ...
  async function handleCheckFamilyList() {
    try {
      const response = await request(`${DEV_API_URL}/members?userId=${encodeURIComponent(loginUserInfo.userInfo.id)}`, "GET")
  ...
  async function handleGetFamilyName(familyIds) {
    ...
    try {
      ...
      for (let i = 0; i < familyIds.length; i++) {
        const response = await request(`${DEV_API_URL}/families/${familyIds[i].family_id}`, "GET")
  ...
  async function handleFindFamilyInfo(payload) {
    try {
      const response = await request(`${DEV_API_URL}/families/find`, "POST", payload)

      if (response.success) {
        const resData = response.data

        if (resData.message === "Families found successfully" && resData.result.length > 0) {
  ...
  async function handleGetFamilyMemberInfo(familyId) {
    try {
      const response = await request(`${DEV_API_URL}/members?familyId=${encodeURIComponent(familyId)}`, "GET")
  ...
  async function handleUpdateMember(nickname) {
    try {
      const response = await request(`${DEV_API_URL}/members/${selectedModalId}`, "PATCH", { nickname: nickname })
  ...
  async function handleDeleteMember(password) {
    try {
      const response = await request(`${DEV_API_URL}/members/${selectedModalId}`, "DELETE", { password: password })
  ...
  async function handleChangePassword(password, newPassword) {
    const isConfirmed = window.confirm("정말 비밀번호를 변경하시겠습니까?\n변경 시 로그아웃됩니다.")
    if (!isConfirmed) {
    ...
    try {
      const response = await request(`${DEV_API_URL}/auth/change-password`, "PATCH", {
        user_id: loginUserInfo.userInfo.id,
